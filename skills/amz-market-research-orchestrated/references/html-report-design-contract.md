...
- Market-depth AI image prompt cards must contain directly usable image-generation prompts, not strategy descriptions. Keep the three fixed prompt slots. The visible prompt body may be English because most image models follow English visual prompts more reliably, but it must describe the actual picture: product concept, scene, composition, lighting/style, visible selling point, and negative constraints such as no text, no watermark, no logo, and no distorted product. Each card must also include Chinese usage notes explaining the SKU strategy and manual verification focus.
...
- Diagnostic child reports must preserve the same stable section anchors used by full reports. Slot cards link to those anchors with `href="#..."`, and detail sections use the matching `id="..."`; dynamic IDs such as `diagnostic-slot-02-cosmo-alexa-标签识别` are invalid because they break navigation and external references.
- Diagnostic slot metrics must be semantic, not index-rotated placeholders. Competitor modules show qualified competitor counts, COSMO/keyword/prompt modules show keyword counts, VOC/demand/user-voice modules show review counts, TikTok modules show TikTok signal counts, supply-chain modules show valid 1688 quote counts, and lifecycle/SKU modules show SKU candidate counts. Repeated generic copy such as one sentence duplicated across every slot is invalid; each slot must state its module-specific evidence gap and recovery action.
...
- Acceptance proof must be bounded. Readiness recovery, render, validator, and optional visual steps must not hang indefinitely; a timed-out recovery step is recorded as a failed step, then the proof continues to diagnostic rendering and validator when diagnostic HTML can still be produced. If `--reference-visual` is requested while the report is only `diagnostic_delivery`, the proof must record a `reference_visual_skipped_reason` instead of silently implying that full visual parity was checked.
...
Diagnostic delivery uses the same COSMO skeleton. Even when data gates fail, the market-depth page must still render all 15 relation cards inside the two product/user lanes, with low-confidence Chinese diagnostics and customer-safe `Pxx` / `Uxx` slot ids in `data-cosmo-relation`. Diagnostic mode must not collapse the matrix to a few example cards and must not expose raw relation codes in customer HTML. The matrix wrapper itself is mandatory: diagnostic HTML must include `class="cosmo-panel cosmo-matrix"` around the matrix lanes; `cosmo-matrix-cell` cards alone do not satisfy the template contract.
...
Customer-visible diagnostic copy must not reuse English template labels or generic filler. Market/VOC diagnostic sections must use Chinese labels such as `负面痛点` and `正面爽点`, not `Pain 主要痛点` or `Joy 主要爽点`. Image prompt diagnostic cards must use Chinese strategy labels such as `图片方案 01`, not visible `PROMPT 01/02/03`. Repeated filler phrases such as `当前只展示...` or `本节只展示...` are invalid because they make the fixed template read as a placeholder shell instead of a business diagnostic.
...
If an allowlisted Amazon image URL is present but the remote image request later fails in the browser, the customer page must keep a stable image frame and switch to a silent neutral placeholder. It must not show customer-visible strings such as `图片加载失败`, `竞品图片未返回`, or `参考竞品图片未返回`. The visual audit may ignore remote image resource 400/403/404 console messages, but page JavaScript errors, layout overflow, empty screenshots, and missing template components must still fail.
...
- If a real Amazon image URL fails at browser load time, preserve the image frame with a silent fallback state instead of adding visible failure text. Image load failure text is an internal runtime state, not customer report copy.
...
- Lifecycle ecosystem must render a three-layer structure: research object -> four-dimensional path (`关联度`, `场景`, `消耗`, `维护`) -> segment/scenario -> SKU candidate or reference ASIN. Internal `Type A/B/C/D` labels must not be visible in customer HTML. Long SKU labels must be truncated or moved to tooltip/detail tables; the Sunburst must not display overlapping long text inside the chart.
- Lifecycle priority display must be adaptive: `<=8` SKU candidates use compact score cards, `9-20` use a horizontal full list, and `>20` use a Top 15 chart plus full candidate-pool table or drawer. Empty 0-100 axes for a small pool are invalid. SKU chart labels must read from stable SKU data fields, not from DOM text that includes image fallback UI such as `图片加载失败`.
