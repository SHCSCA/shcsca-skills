...
def build_report_documents(
    ...
    common = {
        ...
        "{{CONFIDENCE_LEVEL}}": call(fns, "confidence_level", data_pack, analysis_plan),
        "{{CLIENT_TRUST_STRIP}}": call(fns, "client_trust_strip", data_pack, analysis_plan, str(decision)),
    }
    market_replacements = {
        **common,
    ...
    compat_index_replacements = {
        **index_replacements,
        "{{INDEX_CARDS}}": call(fns, "render_index_cards", str(object_value), str(decision), data_pack, link_prefix="html_reports"),
    }
