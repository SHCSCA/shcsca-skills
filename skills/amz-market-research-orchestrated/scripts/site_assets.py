...
(function(){
  ...
  function tableRows(id){
    ...
    return [...table.querySelectorAll('tbody tr')].map((row,idx)=>{
      ...
      return {label,shortSku:shortSkuLabel(label),type:(cells[2]||'A').trim().slice(0,1).toUpperCase(),phase:cells[0]||'-',value:Number.isFinite(value)?value:idx+1};
    });
  }
  ...
  function lifecycleCharts(){
    ...
    chart('sunburst',{tooltip:{trigger:'item',formatter:'{b}: {c} SKU'},nodeClick:false,series:[{type:'sunburst',data:Object.keys(groups).map(type=>({name:`Type ${type}`,value:groups[type].length,itemStyle:{color:typeColors[type]||warningColor},children:groups[type].slice(0,12).map(row=>({name:shortLabel(row.label,14),value:1}))})),radius:[0,'82%'],sort:null,label:{show:false},levels:[{}, {r0:'16%',r:'48%',label:{show:true,rotate:'tangential',fontSize:13,color:'#fff'},itemStyle:{borderWidth:2,borderColor:'#fff'}}, {r0:'48%',r:'82%',label:{show:false},itemStyle:{borderWidth:1,borderColor:'#fff'}}]}]});
    const sorted=skuRows.slice().sort((a,b)=>a.value-b.value).slice(0,18);
    chart('priorityChart',{tooltip:{trigger:'axis',axisPointer:{type:'shadow'},formatter:items=>{const item=Array.isArray(items)?items[0]:items;const row=sorted[item.dataIndex]||{};return `${row.shortSku||row.label}<br/>优先级：${item.value}`;}},grid:{left:190,right:46,bottom:26,top:18,containLabel:false},xAxis:{type:'value',max:100,axisLabel:{fontSize:10,color:'#8a9aaa'},splitLine:{lineStyle:{color:'#f0ede8'}}},yAxis:{type:'category',data:sorted.map(row=>shortSkuLabel(row.label)),axisLabel:{fontSize:11,color:'#5a6a7a',width:170,overflow:'truncate'}},series:[{type:'bar',data:sorted.map(row=>({value:Math.min(100,Math.max(1,row.value)),itemStyle:{color:typeColors[row.type]||accentColor}})),barMaxWidth:16,label:{show:true,position:'right',fontSize:10,color:'#5a6a7a'}}]});
