...
body.template-market>.site-nav,body.template-lifecycle>.site-nav,body.template-demand>.site-nav{display:none}
...
.bar-row{display:grid;grid-template-columns:minmax(120px,1.1fr) minmax(120px,2fr) 48px;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid rgba(26,39,68,.08);transition:background .18s ease}.bar-row:last-child{border-bottom:0}.bar-row>span:first-child{font-weight:800;color:#1a2744}.bar-row>b{font-variant-numeric:tabular-nums;text-align:right;color:#1a2744}.bar{height:8px;background:#ece7df;overflow:hidden}.bar span{display:block;height:100%;width:var(--w,0%);min-width:2px;background:#3d6b9e}.bar.good span{background:#6a9a7a}.bar.bad span{background:#c4705a}.bar.warn span{background:#c9a05a}.bar-row:hover{background:rgba(47,111,143,.08)}.bar-row.is-linked{outline:2px solid rgba(47,111,143,.22)}
...
.template-market #executive-dashboard>.kpi-grid{grid-template-columns:repeat(4,minmax(0,1fr))}
...
.chart-container,.chart{border:1px solid rgba(26,39,68,.12);background:#fff;padding:18px;min-height:220px}.mini-chart{border:1px solid rgba(26,39,68,.12);background:#fff;padding:14px;min-height:0}.card>.mini-chart{margin-top:12px;border-color:rgba(26,39,68,.1);background:#fbfcfd}.template-demand .chart-container,.template-demand .chart,.template-demand .mini-chart{background:#13243d;border-color:#2b4266}
...
(function(){
  ...
  const skipTableEnhancement=(document.documentElement&&document.documentElement.dataset&&document.documentElement.dataset.reportStyle)==='market-depth-report-v2';
  document.querySelectorAll('table').forEach((table,idx)=>{
    if(skipTableEnhancement)return;
  ...
  function datasetRows(section,name){
    ...
    return [...source.querySelectorAll('[data-label]')].map((el,idx)=>{
      ...
      return {label:el.dataset.label||('项目 '+(idx+1)),value:Number.isFinite(value)?value:idx+1};
    });
  }
  function sectionRows(el){
    ...
    return [...section.querySelectorAll('.bar-row')].slice(0,10).map((row,idx)=>{
      const label=row.querySelector('span')?.textContent.trim()||('项目 '+(idx+1));
  ...
  function tableRows(id){
    ...
    return [...table.querySelectorAll('tbody tr')].map((row,idx)=>{
      ...
      const value=cells.map(text=>parseFloat(text.replace(/[^0-9.-]/g,''))).find(Number.isFinite);
      return {label:cells[3]||cells[1]||cells[0]||('SKU '+(idx+1)),type:(cells[2]||'A').trim().slice(0,1).toUpperCase(),phase:cells[0]||'-',value:Number.isFinite(value)?value:idx+1};
    });
  }
  function marketCharts(){
    ...
    chart('priceChart',{tooltip:{trigger:'axis',axisPointer:{type:'shadow'}},grid:{left:'3%',right:'4%',bottom:'3%',containLabel:true},xAxis:{type:'category',data:labels,axisLabel:axisLabelStyle,axisLine:{lineStyle:{color:'#e0ddd8'}}},yAxis:{type:'value',name:'记录数 / 销量',nameTextStyle:axisLabelStyle,axisLabel:axisLabelStyle,splitLine:{lineStyle:{color:'#f0ede8'}}},series:[{name:'清洗数据',type:'bar',barMaxWidth:48,data:values.map((value,idx)=>({value,itemStyle:{color:idx===1?dangerColor:morandiColors[idx%morandiColors.length]}})),label:{show:true,position:'top',fontSize:10,color:'#5a6a7a'}}]});
    ...
    const marginSection=document.getElementById('marginChart').closest('section,.section')||document;
  ...
  function lifecycleCharts(){
    ...
    const groups={};
    skuRows.forEach(row=>{(groups[row.type]||(groups[row.type]=[])).push(row);});
    const typeColors={A:dangerColor,B:accentColor,C:successColor,D:lavenderColor};
    chart('sunburst',{tooltip:{trigger:'item',formatter:'{b}: {c} SKU'},series:[{type:'sunburst',data:Object.keys(groups).map(type=>({name:`Type ${type}`,value:groups[type].length,itemStyle:{color:typeColors[type]||warningColor},children:groups[type].slice(0,12).map(row=>({name:row.label,value:1}))})),radius:[0,'90%'],sort:null,levels:[{}, {r0:'15%',r:'45%',label:{rotate:'tangential',fontSize:13,color:'#fff'},itemStyle:{borderWidth:2,borderColor:'#fff'}}, {r0:'45%',r:'90%',label:{fontSize:11,color:primaryColor,align:'right'},itemStyle:{borderWidth:1,borderColor:'#fff'}}]}]});
    const sorted=skuRows.slice().sort((a,b)=>a.value-b.value).slice(0,18);
    chart('priorityChart',{tooltip:{trigger:'axis',axisPointer:{type:'shadow'}},grid:{left:'3%',right:'8%',bottom:'3%',top:'3%',containLabel:true},xAxis:{type:'value',max:100,axisLabel:{fontSize:10,color:'#8a9aaa'},splitLine:{lineStyle:{color:'#f0ede8'}}},yAxis:{type:'category',data:sorted.map(row=>row.label.length>12?row.label.slice(0,12)+'...':row.label),axisLabel:{fontSize:10,color:'#5a6a7a'}},series:[{type:'bar',data:sorted.map(row=>({value:Math.min(100,Math.max(1,row.value)),itemStyle:{color:typeColors[row.type]||accentColor}})),barMaxWidth:14,label:{show:true,position:'right',fontSize:10,color:'#5a6a7a'}}]});
  ...
  function demandCharts(){
    ...
    const nodeMap=new Map([['$APPEALS',{name:'$APPEALS'}]]);
    const links=[];
...
def attach_site_chrome(html_doc: str, asset_prefix: str = "") -> str:
    ...
    if "site-nav" not in html_doc and not is_child_template:
