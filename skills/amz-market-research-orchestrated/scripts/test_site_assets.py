...
class SiteAssetsTest(unittest.TestCase):
    ...
    def test_chart_and_card_assets_avoid_customer_confusing_labels_and_overflow(self):
        ...
        self.assertIn("SKU 优先级列表", REPORT_JS)
        self.assertIn("priorityEl.classList.add('priority-compact-grid')", REPORT_JS)
        self.assertIn("priority-score-card", REPORT_JS)
        self.assertNotIn("SKU 优先级全量榜", REPORT_JS)
        self.assertIn("row.getAttribute('data-filter')", REPORT_JS)
        self.assertIn("row.dataset.skuLabel", REPORT_JS)
        self.assertIn("图片未返回|竞品图片未返回|参考竞品图片未返回", REPORT_JS)
        self.assertNotIn("图片加载失败", REPORT_JS)
        ...
        self.assertIn("shortLabel(row.shortSku||row.label,12)", REPORT_JS)
        ...
        self.assertIn(".priority-compact-grid", REPORT_CSS)
        self.assertIn(".priority-score-card", REPORT_CSS)
        ...
        self.assertIn("body.diagnostic-page .site-nav{position:sticky", REPORT_CSS)
        self.assertIn(".diagnostic-page .diagnostic-chart{min-height:0", REPORT_CSS)
        self.assertIn(".diagnostic-tabs{display:flex", REPORT_CSS)
        self.assertIn(".template-index .kpi-card .kpi-value{font-size:clamp(18px,2.2vw,30px)", REPORT_CSS)
        self.assertIn("body.template-demand.diagnostic-page{background:#f4f2ef;color:#1a2744}", REPORT_CSS)
        ...
        self.assertIn(".cosmo-top-list ol,.cosmo-gap-panel ul{list-style:none;margin:0;padding:0;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}", REPORT_CSS)
        self.assertIn(".cosmo-top-list li,.cosmo-gap-panel li{display:grid;grid-template-columns:1fr", REPORT_CSS)
        ...
        self.assertIn(".cosmo-matrix-cell{border:0;background:#fbfcfd;padding:14px;min-height:248px;display:grid", REPORT_CSS)
        self.assertIn(".cosmo-card-meta-grid div{border:0;background:transparent", REPORT_CSS)
        self.assertIn(".cosmo-term-block{border:0;background:#f7f9fc", REPORT_CSS)
        self.assertIn(".cosmo-evidence-strip{display:flex;align-items:center;justify-content:space-between;gap:8px;margin:0;padding:6px 8px;background:#f7f6f2;border:0", REPORT_CSS)
        ...
        self.assertIn("body.template-market #cosmo-alexa-tags .cosmo-summary-item{position:relative;border:1px solid #e0ddd8;background:#fff;padding:18px 18px 18px 22px", REPORT_POST_REFERENCE_CSS)
        self.assertIn("body.template-market #cosmo-alexa-tags .cosmo-matrix-cell{border:0;background:#fbfcfd;padding:14px", REPORT_POST_REFERENCE_CSS)
        self.assertIn("body.template-market #cosmo-alexa-tags .cosmo-submodule-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}", REPORT_POST_REFERENCE_CSS)
        self.assertIn("body.template-market #cosmo-alexa-tags .cosmo-top-list ol,body.template-market #cosmo-alexa-tags .cosmo-gap-panel ul{list-style:none;margin:0;padding:0;display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px}", REPORT_POST_REFERENCE_CSS)
        self.assertIn("body.diagnostic-page .evidence-table th,body.diagnostic-page .evidence-table td", REPORT_POST_REFERENCE_CSS)
        self.assertIn("padding:12px 14px!important", REPORT_POST_REFERENCE_CSS)
        self.assertIn(".diagnostic-filter-bar{display:flex;align-items:center;gap:8px;flex-wrap:wrap", REPORT_POST_REFERENCE_CSS)
        self.assertIn(".diagnostic-tabs{display:grid!important;grid-template-columns:repeat(2,max-content) minmax(0,1fr)", REPORT_POST_REFERENCE_CSS)
        self.assertIn(".diagnostic-tabs [data-tab-panel][hidden]{display:none!important}", REPORT_POST_REFERENCE_CSS)
